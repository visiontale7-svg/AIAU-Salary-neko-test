-- A polled provider message can be the owner's own follow-up read back from
-- Devin. The follow-up is already recorded locally when it is sent, so the
-- replayed copy is dropped instead of duplicating it in the run log.
...
begin
  ...
  for v_event in select value from jsonb_array_elements(p_events) loop
    ...
    -- That text is already in the log as `owner_follow_up_attempted`, so
    -- appending it again would show the owner's question twice.
